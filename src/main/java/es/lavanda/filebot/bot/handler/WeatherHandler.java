...
public class WeatherHandler extends TelegramLongPollingBot {
    ...
    private void sendAlerts() {
        ...
        for (WeatherAlert weatherAlert : allAlerts) {
            ...
            try {
                ...
                if (e.getApiResponse().contains("Can't access the chat")
    ...
    private static SendMessage onAlertDeleteOptionSelected(Message message, String language) {
        ...
        if (message.hasText()) {
            if (message.getText().equals(getCancelCommand(language))) {
            ...
            } else if (DatabaseManager.getInstance().getAlertCitiesNameByUser(message.getFrom().getId())
                ...
                sendMessageRequest = sendChooseOptionMessage(message.getChatId(), message.getMessageId(),
                        getAlertsListKeyboard(message.getFrom().getId(), language), language);
            }
        }
    ...
    private static SendMessage onDeleteAlertCommand(Message message, String language) {
        ...
        if (replyKeyboardMarkup != null) {
            ...
            DatabaseManager.getInstance().insertWeatherState(message.getFrom().getId(), message.getChatId(),
                    ALERTDELETE);
        } else {
            sendMessage.setReplyMarkup(getAlertsKeyboard(language));
    ...
    private static SendMessage onUnitsChosen(Integer userId, Long chatId, Integer messageId, String units,
            String language) {
    ...
    private static SendMessage onForecastNewWeather(Message message, String language) {
        if (message.isReply()) {
            return onForecastWeatherReceived(message.getChatId(), message.getFrom().getId(), message.getMessageId(),
                    message.getText(), language);
        } else {
    ...
    private static SendMessage onForecastWeatherCityReceived(Long chatId, Integer userId, Integer messageId,
        ...
        if (cityId != null) {
            ...
            String weather = WeatherService.getInstance().fetchWeatherForecast(cityId.toString(), userId, language,
                    unitsSystem);
            SendMessage sendMessageRequest = new SendMessage();
            sendMessageRequest.enableMarkdown(true);
            sendMessageRequest.setReplyMarkup(getMainMenuKeyboard(language));
    ...
    private static SendMessage onCurrentWeather(Message message, String language) {
        ...
        if (message.hasText()) {
            if (message.getText().startsWith(getNewCommand(language))) {
                ...
                sendMessageRequest = onLocationCurrentWeatherCommand(message.getChatId(), message.getFrom().getId(),
                        message.getMessageId(), language);
            } else if (message.getText().startsWith(getCancelCommand(language))) {
                sendMessageRequest = onCancelCommand(message.getChatId(), message.getFrom().getId(),
                        message.getMessageId(),
                ...
                sendMessageRequest = onCurrentWeatherCityReceived(message.getChatId(), message.getFrom().getId(),
                        message.getMessageId(),
                        message.getText(), language);
            }
        }
    ...
    private static SendMessage onCurrentWeatherCityReceived(Long chatId, Integer userId, Integer messageId, String text,
            String language) {
        Integer cityId = DatabaseManager.getInstance().getRecentWeatherIdByCity(userId, text);
        if (cityId != null) {
            String unitsSystem = DatabaseManager.getInstance().getUserWeatherOptions(userId)[1];
            String weather = WeatherService.getInstance().fetchWeatherCurrent(cityId.toString(), userId, language,
                    unitsSystem);
            SendMessage sendMessageRequest = new SendMessage();
            sendMessageRequest.enableMarkdown(true);
            sendMessageRequest.setReplyMarkup(getMainMenuKeyboard(language));
    ...
    private static SendMessage sendRateMessage(Long chatId, Integer messageId, ReplyKeyboardMarkup replyKeyboardMarkup,
            String language) {
        SendMessage sendMessage = new SendMessage();
        sendMessage.enableMarkdown(true);
        sendMessage.setChatId(chatId);
    ...
    private static SendMessage onForecastWeatherLocationReceived(Message message, String language) {
        ...
        String weather = WeatherService.getInstance().fetchWeatherForecastByLocation(
    ...
    private static SendMessage onForecastWeatherReceived(Long chatId, Integer userId, Integer messageId, String text,
            String language) {
        String unitsSystem = DatabaseManager.getInstance().getUserWeatherOptions(userId)[1];
